...
function fileValidation_to_mtng_no(pdf_data)
{
  ...
  if (year != 'a') {
    for (x in a){
      ...
      console.log(kd)
    if(yd==year){
      document.getElementById("meno").value = mtno;
      
      node.appendChild(textnode);
      if(mtng_val > (mtno) ){
        mtng_val=mtno+1;
        node = document.createElement("Option");
        document.getElementById("meno").appendChild(node);
   ...
   document.getElementById("meno").disabled = false;
  }
  else{
