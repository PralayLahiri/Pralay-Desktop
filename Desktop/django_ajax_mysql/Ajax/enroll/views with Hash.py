...
def check_user(h):
    ...
    # for i in l:
        if h == hashlib.md5(l.encode('utf-8')).hexdigest():
            print("############",hashlib.md5(l.encode('utf-8')).hexdigest())
            return l
...
def login(request):
    ...
    if request.method == "POST" :
        ...
        for id in stud1:    
            uname1 = check_user(hashlib.md5(id[3].encode('utf-8')).hexdigest())
            print("uname1",uname1)
            print("==-=-=-=-=-=-=-=-==-=-=",check_user(hashlib.md5(id[3].encode('utf-8')).hexdigest()))
        print("uname1-----",uname1)
        print("uname-----",psw)
        if len(stud1) != 0 and uname1 == psw:    
            request.session['user'] = uname1
...
def save_data(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print("----------------------------------------")
            print("SID",sid)
            password = check_user(hashlib.md5(password.encode('utf-8')).hexdigest())
            print(check_user(hashlib.md5(password.encode('utf-8')).hexdigest()))
            if sid == '':
