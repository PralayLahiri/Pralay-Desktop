...
def Change_Password(request):
    ...
    if userid is not None:
        ...
        if len(pwd1) != 0:
            ...
            if(id_1 == 2):
                sql = "Update  Users set password = %s where id =2;"
                print("------------------------------------------------------------")
                val = (password)
                print("------------------------------------------------------------")
                cursor.execute(sql,val) 
...
def create_split_pdf(request):
    ...
    if (request.method=="POST" and 'submit' in request.POST):
        ...
        order_type=request.POST.get('submit1')
        #----------------------------------------20220513
        # mtng_no=request.POST.get('mtng_1')
        #----------------------------------------20220513
        #----------------------------------------20220513
        mtng_no1=request.POST.get('mtng_1')
        mtng_no2=request.POST.get('mtng_2')

        agenga_topic1=request.POST.get('agnda_1')
        agenga_topic2=request.POST.get('agnda_2')

        if agenga_topic1 == None or agenga_topic1 == "" :
            agenga_topic=agenga_topic2
        else:
            agenga_topic=agenga_topic1


        if mtng_no2 == None or mtng_no2=="":
              mtng_no = mtng_no1
        else:
            mtng_no = mtng_no2
...
#----------------------------------------20220513-------------------------
            if order_type == 'asc':
                #--------------------------------- original ------------------------------
                if (mtng_no=="ALL" and agenga_topic=="ALL"):
    ...
    for p in showpdf.objects.all():
        ...
        meeting_all.append( p.M_No)
        from_year_list.append(p.pdf_year)
        to_year_list.append( p.pdf_year)

    # for mt in showpdf.objects.values('M_No').distinct():   
    #         meeting_all.append( mt)
            
    meeting_all = list(set(meeting_all)) 
    from_year_list = list(set(from_year_list))  
    to_year_list = list(set(to_year_list))        
    ...
    agenda_list = list(set(agenda_list)) 
    ...
    return render(request,"report_pdf_details.html",context={'pdf':pdf_all, 'mtng_list':meeting_all, 'from_year_list':from_year_list , 'to_year_list':to_year_list, 'agenda_list': agenda_list, 'from_year':from_year, 'to_year':to_year })
...
def reportpdfdata(request):
        ...
        return render(request,"report_pdf_details.html",context={'pdf':pdf_all, 'mtng_list':meeting_all, 'from_year_list':from_year_list , 'to_year_list':to_year_list,  'agenda_list': agenda_list})
